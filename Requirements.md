...
**Structural page-break blocks (all `22`-prefix ctrl_byte variants):** Page boundaries within the body are marked by structural blocks with `B3 ≥ 0x80`, `B4 = 0x0e`, `B5 = 0x07`, `B6 = 0x03` — e.g. `a6 0e 07 03` in `22 6d` files and `c4 0e 07 03` in `22 61` files. These are handled in the main parse loop after the standard structural-header skip is applied: `flush_run(); flush_para(); current_para.page_break_before = True`, then jump to the next `22 ctrl 0b`. Applies to all `22`-prefix ctrl_byte variants; `22 61` files use this mechanism alongside `0e 01`/`0e 02` breaks. Confirmed in BREWERS.5 (12 page breaks) and HENCOTES (7 `c4 0e 07 03` page breaks across 109 `22 61` sample files). Note: originally scoped to `ctrl_byte ≠ 0x61` in error — removed after HENCOTES evidence.
